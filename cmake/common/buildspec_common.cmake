...
function(_setup_obs_studio)
  ...
  if(OS_WINDOWS)
    ...
    if(CMAKE_GENERATOR_TOOLSET)
      set(_cmake_toolset "-T ${CMAKE_GENERATOR_TOOLSET}")
    endif()
  elseif(OS_MACOS)
  ...
  execute_process(
    COMMAND
      ...
      ${_cmake_toolset} -DOBS_CMAKE_VERSION:STRING=3.0.0 -DENABLE_PLUGINS:BOOL=OFF -DENABLE_FRONTEND:BOOL=OFF
...
# _patch_obs_studio: Apply local source compatibility patches before building OBS sources
function(_patch_obs_studio)
  if(NOT OS_WINDOWS OR NOT CMAKE_GENERATOR_TOOLSET MATCHES "ClangCL")
    return()
  endif()

  set(_util_uint64_header "${dependencies_dir}/${_obs_destination}/libobs/util/util_uint64.h")
  if(NOT EXISTS "${_util_uint64_header}")
    return()
  endif()

  file(READ "${_util_uint64_header}" _util_uint64_contents)
  if(_util_uint64_contents MATCHES "obs_plugintemplate_clangcl_udiv128")
    return()
  endif()

  set(
    _clangcl_udiv128_patch
    "#if defined(_MSC_VER) && defined(_M_X64)\n#include <intrin.h>\n#if defined(__clang__)\nstatic inline unsigned __int64 obs_plugintemplate_clangcl_udiv128(unsigned __int64 high,\n\t\t\t\t\t\t\t\t unsigned __int64 low,\n\t\t\t\t\t\t\t\t unsigned __int64 divisor,\n\t\t\t\t\t\t\t\t unsigned __int64 *remainder)\n{\n\tunsigned __int64 quotient = 0;\n\tunsigned __int64 rem = high;\n\n\tfor (int bit = 63; bit >= 0; --bit) {\n\t\tconst int carry = (rem >> 63) != 0;\n\t\trem = (rem << 1) | ((low >> bit) & 1);\n\n\t\tif (carry || rem >= divisor) {\n\t\t\trem -= divisor;\n\t\t\tquotient |= (1ULL << bit);\n\t\t}\n\t}\n\n\tif (remainder)\n\t\t*remainder = rem;\n\n\treturn quotient;\n}\n#define _udiv128 obs_plugintemplate_clangcl_udiv128\n#endif\n#endif"
  )
  ...
  string(
    REPLACE
      "#if defined(_MSC_VER) && defined(_M_X64)\n#include <intrin.h>\n#endif"
      "${_clangcl_udiv128_patch}"
      _util_uint64_contents
      "${_util_uint64_contents}"
  )
  file(WRITE "${_util_uint64_header}" "${_util_uint64_contents}")
endfunction()
